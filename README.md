...
体会,要想完整的把整个过程抽象清楚,需要对整个流程有个最大的认知.
...
将所有`Event`任务,作为一个`Runable`,放到阻塞队列中,让线程池队列执行.

`Event`任务 执行有块有慢, 所以,最终保存到数据库的时候, 并不是按照队列的顺序.
...
另外,用`synchronized`对线程访问方法,进行同步.保证线程顺序执行.
...
在SDK中,采用串行模式,在初始化过程中,`SQLiteDatabase`静态单例, 来保证线程安全.
...
![image.png](https://upload-images.jianshu.io/upload_images/1848340-a77e15a6384ab8ab.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

#### 3.sdk唯一性
为了保证sdk命名唯一性,采用所有必要模块加前缀`E`代表`Event`的处理方式,
避免出现在业务层 查看调用出处的时候,造成误解.比如

![image.png](https://upload-images.jianshu.io/upload_images/1848340-a75d690508ff77e1.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
首先aar包可以将asset等资源打进去,而jar包只会打包编译后的class文件.
sdk中,没有引用其他资源,所有直接采用jar包,更合适.

**2.2 版本管理 是不是可以采用自动打包呢?**
...
task releaseJar(type: Copy) {
    ...
    into( 'build/mylibs')
    include('classes.jar')
    rename('classes.jar', 'jj_analytics_lib_v' + rootProject.ext.versionName +'_release'+ '.jar')
}
